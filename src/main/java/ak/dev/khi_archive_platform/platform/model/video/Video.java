...
@Table(name = "videos",
        indexes = {
                @Index(name = "idx_video_code", columnList = "video_code"),
                @Index(name = "idx_video_project_id", columnList = "project_id"),
                @Index(name = "idx_video_removed_at", columnList = "removed_at")
        })
@Getter
@Setter
@NoArgsConstructor
...
public class Video {
    ...
    /** Business key such as HASAZIRA_VID_RAW_V1_Copy(1)_000001. */
    @Column(name = "video_code", unique = true, nullable = false, length = 255)
    private String videoCode;

    /** Video belongs to a project (collection). */
    ...
    @CollectionTable(name = "video_subjects", joinColumns = @JoinColumn(name = "video_id"))
    ...
    @Column(name = "person_shown_in_video", columnDefinition = "TEXT")
    private String personShownInVideo;

    @Builder.Default
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "video_colors", joinColumns = @JoinColumn(name = "video_id"))
    @Column(name = "color", columnDefinition = "TEXT")
    private List<String> colorOfVideo = new ArrayList<>();

    /**
     * Video version type: RAW, MASTER, RESTORED, ARCHIVE, ORIGINAL,
    ...
    @Builder.Default
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "video_usages", joinColumns = @JoinColumn(name = "video_id"))
    @Column(name = "usage_context", columnDefinition = "TEXT")
    private List<String> whereThisVideoUsed = new ArrayList<>();
    ...
    @Column(name = "frame_rate", length = 100)
    private String frameRate;

    @Column(name = "overall_bit_rate", length = 100)
    ...
    @Column(name = "audio_channels", length = 100)
    ...
    @Column(name = "creator_artist_director", columnDefinition = "TEXT")
    ...
    @Builder.Default
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "video_keywords", joinColumns = @JoinColumn(name = "video_id"))
    ...
    @Column(name = "video_file_url", length = 1000)
